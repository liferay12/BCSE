...
<%--	<input type="file" name="<portlet:namespace/><%=namePrefix+"_file"%>" id="<portlet:namespace/><%=elementName+"_file"%>">--%>
	...
	<%--<aui:input type="file" name="<%=elementName+"_file"%>" style="width:0px !important; height:0px !important; line-height:0px !important; border: 0px !important;" label="">
	</aui:input>--%>
	<span class="text-danger font-weight-bold" id="<portlet:namespace /><%=elementName+"_errorMessage"%>"></span>
...
<%
	...
	if(!thumbnail){
%>		
		<span id="<portlet:namespace /><%=elementName+"_displayFileName"%>" class="display:block;"><%=displayFileName %></span>
<%		
		if(!readOnly){	
%>			
		<%-- 	<button type="button" class="btn btn-primary btn-sx" id="<portlet:namespace /><%=elementName+"_uploadBtn"%>" style="font-size:1em;<%=(readOnly?"display:none;":"")%>" value="Choose File" ><i class="icon-cloud-upload mr-2"></i>Upload File</button>
 --%><%
		}
	}else{
%>	
		<div style="width:<%=width+2%>px;height:<%=height+2%>px; border:1px solid #ced4da; text-align:center; padding:2px;" class="mb-2 mt-2">
			<img id="<portlet:namespace/><%=elementName%>_thumbnail" src="<%=placeHolder%>" style="width:100%; height: 100%;"/>
<%		
		if(!readOnly){	
%>			
			<%-- <button type="button" class="btn btn-primary btn-sx" id="<portlet:namespace /><%=elementName+"_uploadBtn"%>" style="font-size:1em;<%=(readOnly?"display:none;":"")%>margin-top: -<%=height*0.4%>%;" value="Choose File" >Upload File</button>
			<button type="button" class="btn btn-danger btn-sx ml-2 pl-2 pr-2" id="<portlet:namespace /><%=elementName+"_deleteBtn"%>" style="font-size:1em; display:none;  padding-left:12px !important; padding-right:12px !important; margin-top:25px !important;"><i class="icon-trash"></i></button>
 --%><%
		}
%>			
		</div>
...
<aui:input type="text" name="<%=elementName+"_fileName"%>" value="<%=fileName%>" label="" style="
    margin-top: -39px !important;
    height: 0px !important;
    padding-top: 0px !important;
    padding-bottom: 0px !important;
    border: none !important;" readonly="true">
<%
	if(required){
%>
	<aui:validator name="required" errorMessage="*Uploading of the document is required!">
		function() {
			...
			if(consumerType=="<%=consumerType%>" && consumerType=="Firm"){
				return true;
			}
        	return false;
        }
...
<button type="button" class="btn btn-success btn-sx ml-2 pl-2 pr-2" id="<portlet:namespace /><%=elementName+"_viewBtn"%>" style="font-size:1em; display:none; padding:5px 12px 5px 12px!important;"><i class="icon-download-alt"></i></button>
<%-- <button type="button" class="btn btn-danger btn-sx ml-2 pl-2 pr-2" id="<portlet:namespace /><%=elementName+"_deleteBtn"%>" style="font-size:1em; display:none; padding:5px 12px 5px 12px!important;"><i class="icon-trash"></i></button>
 --%></div>
<%
	}
%>
...
<%--	A.one('#<portlet:namespace /><%=elementName%>_uploadBtn').on('click', function(event) {
	...
	$('#<portlet:namespace/><%=elementName%>_file').on('change', function(event) {
		...
		uploadFile('<%=connectionRequestId%>', connectionDocumentId, '<%=documentType%>', $('#<portlet:namespace/><%=elementName%>_documentName').val(), $('#<portlet:namespace/><%=elementName%>_file'), <%=maxFileSize%>, $('#<portlet:namespace/><%=elementName%>_acceptTypes').val(),'', successFunc);
	});
...
%>
	...
	Liferay.provide(window,'<portlet:namespace/><%=elementName+"_deleteDoc"%>', function () {
		try{
			if($('#<portlet:namespace/><%=elementName%>_fileName').val() ==""){
				return;
			}
			var successFunc=<portlet:namespace /><%=elementName%>_onDeleteSuccess;
			deleteConnectionDocument($('#<portlet:namespace/><%=elementName%>_connectionDocumentId').val(), successFunc);
