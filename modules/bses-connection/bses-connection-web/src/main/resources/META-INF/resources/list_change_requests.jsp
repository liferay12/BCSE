...
<%
	String loginId=(String)portletSession.getAttribute(BsesConnectionPortletKeys.CA_NUMBER);
	String mobileNo=(String)portletSession.getAttribute(BsesConnectionPortletKeys.MOBILE_NO);
	String emailId=(String)portletSession.getAttribute(BsesConnectionPortletKeys.EMAIL_ID);
%>
<portlet:renderURL var="newChangeRequestURL">
	<portlet:param name="mvcPath" value="/change_request_options.jsp" />
...
<portlet:renderURL var="listConnectionURL">
	<portlet:param name="mvcPath" value="/list_change_requests.jsp" />
	<portlet:param name="mobileNo" value="<%=mobileNo%>" />
	<portlet:param name="emailId" value="<%=emailId%>" />
</portlet:renderURL>
...
<portlet:actionURL name="editChangeRequestAction" var="editChangeRequestActionURL"/>

<%
	//session.removeAttribute(ConnectionRequest.class.getName()+"#id");
	List<ConnectionRequest> connectionRequestList=null;//(List)request.getAttribute(ConnectionRequest.class.getName()+"#list");
	//if(connectionRequestList==null){
	connectionRequestList=ConnectionRequestLocalServiceUtil.getChangeRequestsByCaNumber(loginId);
	//}
...
<div class="card card-primary bg-light mb-2">
	<div class="card-header">
		<div class="container-fluid p-1">
			<div class="row">
				<div class="col-md-6">
					<h6><liferay-ui:message key="list-change-request-title" /></h6>
				</div>
...
%>	
				...
				<button class="btn-primary text-white p-2 ml-3" id="<portlet:namespace/>createChangeRequest" value="New Connection">New Request</button>
<%
	}
%>				
			</div>
...
<%
				for(ConnectionRequest r:connectionRequestList){
					ConnectionRequest or =ConnectionRequestLocalServiceUtil.convertToConnectionRequest(r.getOldData());
					String name="";
					System.out.println("StringUtils.isBlank(r.getFirstName() - "+StringUtils.isBlank(r.getFirstName()));
					if(StringUtils.isBlank(r.getFirstName()) && or != null){
						 name=(StringUtils.isNotBlank(or.getTitle())? MasterData.getTitleValue(or.getTitle()):StringPool.BLANK)+StringPool.SPACE+or.getFirstName()
							 +(StringUtils.isNotBlank(or.getMiddleName())?StringPool.SPACE+or.getMiddleName():StringPool.BLANK)
						 	+(StringUtils.isNotBlank(or.getLastName())?StringPool.SPACE+or.getLastName():StringPool.BLANK);
					}else {
						name=(StringUtils.isNotBlank(r.getTitle())? MasterData.getTitleValue(r.getTitle()):StringPool.BLANK)+StringPool.SPACE+r.getFirstName()
							+(StringUtils.isNotBlank(r.getMiddleName())?StringPool.SPACE+r.getMiddleName():StringPool.BLANK)
							+(StringUtils.isNotBlank(r.getLastName())?StringPool.SPACE+r.getLastName():StringPool.BLANK);
					}
					...
					<tr>
						...
						<%}else{ %>
						<span class="text-primary"> <%=r.getOrderNo() %></a>
							...
							<span class="btn-group ml-2" style="display:none">
...
<%
						String requestType="";
						if(StringUtils.equals(r.getRequestType(),"U0304") || StringUtils.equals(r.getRequestType(),"U03") || StringUtils.equals(r.getRequestType(),"U04")){
							requestType="Load Change";
						}else if(StringUtils.equals(r.getRequestType(),"U0506")  || StringUtils.equals(r.getRequestType(),"U05") || StringUtils.equals(r.getRequestType(),"U06")){
							requestType="Category Change";
						}else{
							requestType=MasterData.getRequestType(r.getRequestType());
						}
%>						
						<td><%=requestType%></td>
	...
	<div class="modal-dialog modal-dialog-centered">
		<div class="modal-content">
			<div class="modal-header bg-danger">
				<h5 class="modal-title font-weight-bold text-white">Delete Change Request?</h5>
...
<liferay-util:include page="/change_request_options.jsp" servletContext="<%=application%>">
...
%>
		...
		$("#<portlet:namespace/>createChangeRequest").click(function(){
			$('#connection-mode-type-modal').modal('show');
		});		
		
		$(".edit-connection-btn").click(function(){
			window.location.href='<%=editChangeRequestActionURL.toString()%>&<portlet:namespace/>connectionRequestId='+$(this).attr("data-id");
		});
